...
| 00.1 | Navigate to `/member` | Member list loads | Verify existing members: Admin, Father, Mother should already be seeded. If not, create them via "+ New Member". |
| 00.2 | If fewer than 3 members exist, create them: | | |
| 00.3 | Click "+ New Member", Name="Admin", Short Name="EF", click "Add Member" | Member created | Admin card appears in list. |
| 00.4 | Click "+ New Member", Name="Father", Short Name="AZ", click "Add Member" | Member created | Father card appears. |
| 00.5 | Click "+ New Member", Name="Mother", Short Name="NA", click "Add Member" | Member created | Mother card appears. |
| 00.6 | Click "+ New Member", Name="Test Borrower", Short Name="TB", click "Add Member" | Member created | Test Borrower card appears. |
| **TS-00B — Add Accounts to Admin (member 1)** | | | |
| 00.7 | Navigate to `/member/1` (Admin's profile) | Profile loads | |
| 00.8 | Click "+ Add" in Accounts section | Add Account modal opens | |
| 00.9 | Name="Admin Cash", Type="Cash", Balance="50000", click "Add Account" | Account created | Appears in carousel with gold chip (cash type). |
| 00.10 | Click "+ Add", Name="Admin bKash", Type="Mobile Wallet", Balance="25000", click "Add Account" | Account created | |
| 00.11 | Click "+ Add", Name="Admin Bank", Type="Bank", Balance="100000", click "Add Account" | Account created | |
| 00.12 | Click "+ Add", Name="Admin Business", Type="Business", Balance="75000", click "Add Account" | Account created | |
| 00.13 | Click "+ Add", Name="Admin Savings", Type="Savings", Balance="30000", click "Add Account" | Account created | |
| **TS-00C — Add Accounts to Father (member 2)** | | | |
| 00.14 | Navigate to `/member/2` (Father's profile) | | |
| 00.15 | Click "+ Add", Name="Father Cash", Type="Cash", Balance="20000", click "Add Account" | Account created | |
| 00.16 | Click "+ Add", Name="Father Bank", Type="Bank", Balance="80000", click "Add Account" | Account created | |
| 00.17 | Click "+ Add", Name="Father bKash", Type="Mobile Wallet", Balance="15000", click "Add Account" | Account created | |
| **TS-00D — Add Accounts to Mother (member 3)** | | | |
| 00.18 | Navigate to `/member/3` (Mother's profile) | | |
| 00.19 | Click "+ Add", Name="Mother Bank", Type="Bank", Balance="60000", click "Add Account" | Account created | |
| 00.20 | Click "+ Add", Name="Mother Cash", Type="Cash", Balance="10000", click "Add Account" | Account created | |
...
| 00.30 | Pick source = Admin's Bank (lender), destination = Father's Bank (borrower), amount=30000, desc="Internal loan: Admin→Father" | Fields filled | |
| 00.31 | Click "Issue Internal Loan" | Loan created | Modal closes. |
| 00.32 | Open modal → Loan → Internal. Source = Admin Cash, Destination = Mother Bank, amount=15000, desc="Internal loan: Admin→Mother" | Loan created | |
| 00.33 | Open modal → Loan → Internal. Source = Father Bank, Destination = Test Borrower's Bank, amount=10000, desc="Internal loan: Father→TB" | Loan created | |
| **TS-00H — Issue External Loans** | | | |
| 00.34 | Open modal → Loan → External → "Give" | Give mode | Source + Debtor counterparty. |
| 00.35 | Pick source = Admin bKash, debtor = External Debtor A, amount=20000, desc="Loan to External Debtor A" | | |
| 00.36 | Click "Issue Loan" | External loan created | |
| 00.37 | Open modal → Loan → External → "Give". Source = Admin Bank, debtor = External Debtor A, amount=10000, desc="Second loan to Debtor A" | Another loan | Stack for External Debtor A now has 2 loans. |
| **TS-00I — Add Regular Transactions (Income & Expense)** | | | |
| 00.38 | Open modal → "Income". Source = Admin Business, amount=50000, desc="Business revenue" | | |
| 00.39 | Click "Complete Income" | Income created | |
| 00.40 | Open modal → "Income". Source = Father Bank, amount=25000, desc="Salary deposit" | | |
| 00.41 | Click "Complete Income" | Income created | |
| 00.42 | Open modal → "Expense". Source = Admin Cash, amount=3000, desc="Groceries" | | |
| 00.43 | Click "Complete Expense" | Expense created | |
| 00.44 | Open modal → "Expense". Source = Father bKash, amount=1500, desc="Mobile recharge" | | |
| 00.45 | Click "Complete Expense" | Expense created | |
| 00.46 | Open modal → "Expense". Source = Admin Bank, amount=10000, desc="Utility bills" | | |
| 00.47 | Click "Complete Expense" | Expense created | |
| **TS-00J — Record a Transfer between members** | | | |
| 00.48 | Open modal → "Transfer". Source = Admin Cash, Destination = Mother Bank, amount=5000, desc="Transfer to Mother" | | |
...
| 00.52 | Navigate to `/member/1` (Admin) | Ledger shows all transactions | Income, expense, transfer, internal loan issue, external loan issue all visible. Filter tabs work. |
| 00.53 | Navigate to `/member/2` (Father) | Ledger shows transactions | Internal loan received, income, expense visible. |
| 00.54 | Navigate to `/member/3` (Mother) | Ledger shows transactions | Internal loan received, transfer received visible. |
...
| 1.5 | Click "Member Profile" card | Navigates to `/member/1` | URL changes to `/member/1`. Member profile for Admin loads. |
...
| # | Step | Expected | Check |
|---|------|----------|-------|
| 3.1 | Member list loads | Cards for non-external members | Header "Family Members" + count. Grid of member cards (Admin, Father, Mother). Each card: avatar (48px) + name + "Member" tag + net balance. |
| 3.2 | Click Admin's card | Navigates to `/member/1` | URL is `/member/1`. Admin's profile loads. |
| 3.3 | Navigate back to `/member` | | |
| 3.4 | Click Father's card | Navigates to `/member/2` | URL is `/member/2`. Father's profile loads. |
| 3.5 | Navigate back to `/member` | | |
| 3.6 | Click Mother's card | Navigates to `/member/3` | URL is `/member/3`. Mother's profile loads. |
...
**Precondition:** Navigate to Admin's profile at `/member/1`.

| # | Step | Expected | Check |
|---|------|----------|-------|
| 4.0 | Desktop >=1024px — expand this test for desktop layout | | |
| 4.1 | Profile loads | Profile card visible | Avatar (72px) + name "Admin" + tag "Family" + net balance (animated). |
