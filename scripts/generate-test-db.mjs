...
// ── Members ────────────────────────────────────────────────────────────────
const members = [
  { name: 'এফটি নূর', short: 'Admin', email: 'admin@example.com', phone: '01711-111111', ext: 0 },
...
const ADMIN_DBB = accIds[0];
const ADMIN_BKASH = accIds[1];
const ADMIN_CASH = accIds[2];
...
// -- Salary income (Admin) --
insertTx('income', 'বেতন - জানুয়ারি ২০২৬', 85000, null, ADMIN_DBB, memIds[0], '2026-01-05');
insertTx('income', 'বেতন - ফেব্রুয়ারি ২০২৬', 85000, null, ADMIN_DBB, memIds[0], '2026-02-05');
insertTx('income', 'বেতন - মার্চ ২০২৬', 90000, null, ADMIN_DBB, memIds[0], '2026-03-05');
insertTx('income', 'ফ্রিল্যান্সিং আয়', 45000, null, ADMIN_DBB, memIds[0], '2026-03-12');
insertTx('income', 'বেতন - এপ্রিল ২০২৬', 90000, null, ADMIN_DBB, memIds[0], '2026-04-05');
insertTx('income', 'বেতন - মে ২০২৬', 90000, null, ADMIN_DBB, memIds[0], '2026-05-05');
insertTx('income', 'বেতন - জুন ২০২৬', 95000, null, ADMIN_DBB, memIds[0], '2026-06-05');

// -- Expenses --
insertTx('expense', 'বাজার খরচ', 8500, ADMIN_CASH, null, memIds[0], '2026-01-08');
insertTx('expense', 'ইলেকট্রিক বিল', 4200, ADMIN_DBB, null, memIds[0], '2026-01-12');
insertTx('expense', 'বাজার খরচ', 7200, ADMIN_CASH, null, memIds[0], '2026-02-10');
insertTx('expense', 'গ্যাস বিল', 1850, ADMIN_DBB, null, memIds[0], '2026-02-15');
insertTx('expense', 'পরিবার ভ্রমণ - কক্সবাজার', 25000, ADMIN_DBB, null, memIds[0], '2026-03-20');
insertTx('expense', 'ঔষধ', 2300, ADMIN_CASH, null, memIds[0], '2026-04-05');
insertTx('expense', 'বাজার খরচ', 9100, ADMIN_CASH, null, memIds[0], '2026-04-12');
insertTx('expense', 'মোবাইল রিচার্জ', 500, ADMIN_BKASH, null, memIds[0], '2026-04-15');
insertTx('expense', 'বাজার খরচ', 7800, ADMIN_CASH, null, memIds[0], '2026-05-08');
insertTx('expense', 'ঈদ বাজার', 32000, ADMIN_CASH, null, memIds[0], '2026-05-20');
insertTx('expense', 'ডাক্তার ফি', 1500, ADMIN_CASH, null, memIds[0], '2026-06-02');
insertTx('expense', 'বাজার খরচ', 6500, ADMIN_CASH, null, memIds[0], '2026-06-10');

// -- Transfers --
insertTx('transfer', 'এটিএম থেকে টাকা তোলা', 15000, ADMIN_DBB, ADMIN_CASH, memIds[0], '2026-01-06');
insertTx('transfer', 'বিকাশে টাকা', 10000, ADMIN_DBB, ADMIN_BKASH, memIds[0], '2026-02-01');
insertTx('transfer', 'এটিএম থেকে টাকা তোলা', 20000, ADMIN_DBB, ADMIN_CASH, memIds[0], '2026-03-01');
insertTx('transfer', 'বিকাশে টাকা', 8000, ADMIN_DBB, ADMIN_BKASH, memIds[0], '2026-04-01');
insertTx('transfer', 'নুসরাতকে নগদ', 10000, ADMIN_CASH, NUSRAT_CASH, memIds[0], '2026-04-10');
insertTx('transfer', 'এটিএম থেকে টাকা তোলা', 12000, ADMIN_DBB, ADMIN_CASH, memIds[0], '2026-05-01');
insertTx('transfer', 'বিকাশ থেকে নগদ', 5000, ADMIN_BKASH, ADMIN_CASH, memIds[0], '2026-05-15');
insertTx('transfer', 'নাগাদে টাকা', 7000, ADMIN_DBB, NUSRAT_NAGAD, memIds[0], '2026-05-22');
insertTx('transfer', 'বাবার জন্য নগদ', 5000, ADMIN_DBB, SHAHID_CASH, memIds[2], '2026-06-01');
insertTx('transfer', 'এটিএম থেকে টাকা তোলা', 10000, ADMIN_DBB, ADMIN_CASH, memIds[0], '2026-06-05');
...
// Loan 1: Admin lends 50000 to Rafiq
...
db.run(
  ...
  { $1: loan1Id, $2: ADMIN_DBB, $3: RAFIQ_ACC, $4: now },
);
insertTx('lend', 'রফিককে ঋণ দিয়েছি', 50000, ADMIN_DBB, RAFIQ_ACC, memIds[0], '2026-01-15', loan1Id);

// Loan 2: Admin lends 30000 to Jashim
const loan2Id = id('loan');
db.run(
  `INSERT INTO loans (id,lender_account_id,borrower_account_id,principal,outstanding,status,description,created_at,updated_at)
   VALUES ($1,$2,$3,30000,15000,'active','জরুরি প্রয়োজনে ঋণ',$4,$4)`,
  { $1: loan2Id, $2: ADMIN_DBB, $3: JASHIM_ACC, $4: now },
);
insertTx('lend', 'জসিমকে ঋণ দিয়েছি', 30000, ADMIN_DBB, JASHIM_ACC, memIds[0], '2026-02-10', loan2Id);
...
insertTx('repay', 'রফিকের কিস্তি', 10000, RAFIQ_ACC, ADMIN_DBB, memIds[0], '2026-02-15', loan1Id);
setOutstanding(loan1Id, 40000);
insertTx('repay', 'রফিকের কিস্তি', 10000, RAFIQ_ACC, ADMIN_DBB, memIds[0], '2026-04-10', loan1Id);
setOutstanding(loan1Id, 30000);
insertTx('repay', 'জসিমের কিস্তি', 10000, JASHIM_ACC, ADMIN_DBB, memIds[0], '2026-03-15', loan2Id);
setOutstanding(loan2Id, 20000);
insertTx('repay', 'জসিমের কিস্তি', 5000, JASHIM_ACC, ADMIN_DBB, memIds[0], '2026-05-10', loan2Id);
...
// -- Internal loan (Admin lends to Shahid) --
const loan4Id = id('loan');
db.run(
  `INSERT INTO loans (id,lender_account_id,borrower_account_id,principal,outstanding,status,description,created_at,updated_at)
   VALUES ($1,$2,$3,30000,0,'settled','বাবাকে দেওয়া ঋণ (পরিশোধিত)',$4,$4)`,
  { $1: loan4Id, $2: ADMIN_DBB, $3: SHAHID_CASH, $4: now },
);
insertTx('lend', 'বাবাকে ঋণ', 30000, ADMIN_DBB, SHAHID_CASH, memIds[0], '2026-01-20', loan4Id, memIds[2]);
insertTx('repay', 'বাবার ঋণ শোধ', 30000, SHAHID_CASH, ADMIN_DBB, memIds[2], '2026-03-10', loan4Id, memIds[2]);
...
insertTx('income', 'বেতন - জুলাই ২০২৬', 95000, null, ADMIN_DBB, memIds[0], '2026-07-05');
insertTx('expense', 'বাড়ি ভাড়া', 15000, ADMIN_DBB, null, memIds[0], '2026-07-01');
