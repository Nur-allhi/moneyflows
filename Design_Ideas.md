...
### A. The Dashboard (`/`)
* **Profile Swapper (Header)**: A horizontal profile selection bar at the top showing avatars for `Admin`, `Admin Business`, `Father (Father)`, and `Mother (Mother)`.
* **Top Metric Grid**:
...
* **Two-Column Lower Section**:
  * **Left Column (Custom Balance Groups)**: Shows grouped account cards (e.g., "Combined Brac Bank" with Admin's and Father's Brac bank cards merged).
...
* Dynamically displays field options based on tab selection:
  ...
  * Choosing **Loan** displays a `Borrower` list (including shortcut buttons to add new debtors like External Debtor A/BTC directly).
